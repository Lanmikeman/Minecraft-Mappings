...
public final class RawMesh {
	...
	public void generateNormals() {
		...
		faces.forEach(face -> {
			if (face.vertices.length >= 3) {
				...
				final double ax = vertices.get(i1).position.data.x - vertices.get(i0).position.data.x;
				final double ay = vertices.get(i1).position.data.y - vertices.get(i0).position.data.y;
				final double az = vertices.get(i1).position.data.z - vertices.get(i0).position.data.z;
				final double bx = vertices.get(i2).position.data.x - vertices.get(i0).position.data.x;
				final double by = vertices.get(i2).position.data.y - vertices.get(i0).position.data.y;
				final double bz = vertices.get(i2).position.data.z - vertices.get(i0).position.data.z;
				final double nx = ay * bz - az * by;
				final double ny = az * bx - ax * bz;
				final double nz = ax * by - ay * bx;
				final double t1 = nx * nx + ny * ny + nz * nz;
				if (t1 != 0) {
					final double t2 = 1 / Math.sqrt(t1);
	...
	public void upload(Mesh mesh, VertexAttributeMapping mapping) {
		distinct();
		final ByteBuffer vertexBuffer = OffHeapAllocator.allocate(vertices.size() * mapping.strideVertex);
		vertices.forEach(vertex -> {
			if (shouldWriteVertexBuffer(mapping, VertexAttributeType.POSITION)) vertexBuffer.putFloat(vertex.position.data.x).putFloat(vertex.position.data.y).putFloat(vertex.position.data.z);
			if (shouldWriteVertexBuffer(mapping, VertexAttributeType.COLOR)) vertexBuffer.putInt(vertex.color);
			if (shouldWriteVertexBuffer(mapping, VertexAttributeType.UV_TEXTURE)) vertexBuffer.putFloat(vertex.u).putFloat(vertex.v);
			if (shouldWriteVertexBuffer(mapping, VertexAttributeType.UV_LIGHTMAP)) vertexBuffer.putInt(vertex.light);
			if (shouldWriteVertexBuffer(mapping, VertexAttributeType.NORMAL)) {
				...
				vertexBuffer.put((byte) (normal.data.x * 0x7F)).put((byte) (normal.data.y * 0x7F)).put((byte) (normal.data.z * 0x7F));
			}
			if (mapping.paddingVertex > 0) vertexBuffer.put((byte) 0);
		});
		mesh.vertexBuffer.upload(vertexBuffer, VertexBuffer.USAGE_STATIC_DRAW);
		OffHeapAllocator.free(vertexBuffer);

		final ByteBuffer indexBuffer = OffHeapAllocator.allocate(faces.size() * 3 * Integer.BYTES);
		faces.forEach(face -> {
			for (final int vertexIndex : face.vertices) indexBuffer.putInt(vertexIndex);
		});
	...
	public Mesh upload(VertexAttributeMapping mapping) {
		validateVertexIndex();
	...
	private static boolean shouldWriteVertexBuffer(VertexAttributeMapping mapping, VertexAttributeType type) {
		return mapping.sources.get(type) == VertexAttributeSource.VERTEX_BUFFER;
	}
	...
	public void applyRotation(Vector3f axis, float angle) {
		...
		vertices.forEach(vertex -> {
			vertex.position.data.rotateAxis(rad, axis.data.x, axis.data.y, axis.data.z);
			vertex.normal.data.rotateAxis(rad, axis.data.x, axis.data.y, axis.data.z);
		});
	}
	...
	public void applyScale(float x, float y, float z) {
		...
		vertices.forEach(vertex -> {
			...
			final float nx2 = vertex.normal.data.x * vertex.normal.data.x;
			final float ny2 = vertex.normal.data.y * vertex.normal.data.y;
			final float nz2 = vertex.normal.data.z * vertex.normal.data.z;
			final float u1 = nx2 * rx2 + ny2 * ry2 + nz2 * rz2;
			if (u1 != 0) {
				final float u2 = (float) Math.sqrt((nx2 + ny2 + nz2) / u1);
	...
	public void applyShear(Vector3f dir, Vector3f shear, float ratio) {
		vertices.forEach(vertex -> {
			final float n1 = ratio * (dir.data.x * vertex.position.data.x + dir.data.y * vertex.position.data.y + dir.data.z * vertex.position.data.z);
			final Vector3f offset1 = Utilities.copy(shear);
			offset1.data.mul(n1);
			vertex.position.data.add(offset1.data);
			if (!vectorIsZero(vertex.normal)) {
				final float n2 = ratio * (shear.data.x * vertex.normal.data.x + shear.data.y * vertex.normal.data.y + shear.data.z * vertex.normal.data.z);
	...
	private static boolean vectorIsZero(Vector3f vector3f) {
		return vector3f.data.x == 0 && vector3f.data.y == 0 && vector3f.data.z == 0;
	}
}

...
public class EventRegistry extends DummyClass {
	...
	public void registerStartWorldTick(Consumer<ServerWorld> consumer) {
		ServerTickEvents.START_LEVEL_TICK.register(serverWorld -> consumer.accept(new ServerWorld(serverWorld)));
	}

	@MappedMethod
	public void registerEndWorldTick(Consumer<ServerWorld> consumer) {
		ServerTickEvents.END_LEVEL_TICK.register(serverWorld -> consumer.accept(new ServerWorld(serverWorld)));
	}
	...
	public void registerChunkLoad(BiConsumer<ServerWorld, WorldChunk> consumer) {
		ServerChunkEvents.CHUNK_LOAD.register((serverWorld, worldChunk, newChunk) -> consumer.accept(new ServerWorld(serverWorld), new WorldChunk(worldChunk)));
	}

...
public final class ObjModelLoader {
	...
	public static Map<String, List<RawMesh>> loadModel(String objString, Function<String, String> mtlResolver, Function<String, Identifier> textureResolver, AtlasManager atlasManager, boolean splitModel) {
		...
		try {
			final Obj sourceObj = ObjReader.read(new ByteArrayInputStream(objString.getBytes(StandardCharsets.UTF_8)));

			final Map<String, Mtl> materials = new HashMap<>();
			sourceObj.getMtlFileNames().forEach(mtlFileName -> {
				try {
					final String resolved = mtlResolver.apply(mtlFileName.replace("\\\\", "/").replace("\\", "/"));
					if (resolved != null && !resolved.isEmpty()) {
						MtlReader.read(new ByteArrayInputStream(resolved.getBytes(StandardCharsets.UTF_8))).forEach(mtl -> materials.put(mtl.getName(), mtl));
					}
				} catch (Exception e) {
			...
			if (obj.getNumFaces() > 0) {
				...
				if (!materials.isEmpty()) {
					...
					if (objMaterial == null) {
						...
						final String mapKd = objMaterial.getMapKd();
						if (mapKd == null || mapKd.isEmpty()) {
							texture = "";
						} else {
							texture = mapKd;
						}
						final FloatTuple kd = objMaterial.getKd();
						color = kd == null ? mergeColor(0xFF, 0xFF, 0xFF, 0xFF) : mergeColor(
								(int) (kd.getX() * 0xFF), (int) (kd.getY() * 0xFF), (int) (kd.getZ() * 0xFF),
								objMaterial.getD() == null ? 0xFF : (int) (objMaterial.getD() * 0xFF)
						);
					}
				} else {
					texture = "_".equals(materialGroupName) ? "" : materialGroupName;
	...
	private static int mergeColor(int r, int g, int b, int a) {
		// Minecraft / ColorModulator expect ARGB. Older custom shaders used RRGGBBAA;
		// with stock entity ColorModulator that made dark MTL metals (couplers) near-black.

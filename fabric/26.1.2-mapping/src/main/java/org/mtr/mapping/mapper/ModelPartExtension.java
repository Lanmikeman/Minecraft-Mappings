...
public final class ModelPartExtension extends DummyClass {
	...
	public ModelPartExtension addChild() {
		...
		final ModelPartExtension modelPartExtension = new ModelPartExtension(
				name,
				modelPartData.addOrReplaceChild(name, CubeListBuilder.create(), getModelTransform())
		);
	...
	public void addCuboid(float x, float y, float z, int sizeX, int sizeY, int sizeZ, float inflation, boolean mirrored) {
		setChild();
		final String name = getRandomPartName();
		final CubeListBuilder modelPartBuilder = CubeListBuilder.create()
				.mirror(mirrored)
				.addBox(name, x, y, z, sizeX, sizeY, sizeZ, new CubeDeformation(inflation), tempU, tempV);
	...
	private void setChild() {
		if (!childAdded) {
			modelPartData = modelPartData.addOrReplaceChild(
					getLastName(),
	...
	private PartPose getModelTransform() {
		return PartPose.offsetAndRotation(
				tempPivotX, tempPivotY, tempPivotZ,
				tempRotationX, tempRotationY, tempRotationZ
		);
	}
